...
const convertTypes = (typ: string) => {
  if(typ === "number") return "float";
  return typ
}

const compile = (node: ts.Node, file: ts.SourceFile): string => {

  if(ts.isInterfaceDeclaration(node)) { 
    let properties: ts.Node[] = []
    let other: ts.Node[] = []
